...
export class ProgrammingLanguageResolver {
  ...
  async createPLanguage(
    @Arg('options') options: ProgrammingLanguageInput
  ): Promise<ProgrammingLanguage> {
    const pLanguage = await ProgrammingLanguage.create(options).save();
    return pLanguage;
  }
  ...
  async updatePLanguage(
    ...
    @Arg('input', () => ProgrammingLanguageUpdateInput)
